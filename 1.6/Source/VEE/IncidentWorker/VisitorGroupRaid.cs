...
namespace VEE.RegularEvents
{
    ...
    public class VisitorGroupRaid : IncidentWorker_NeutralGroup
    {
        ...
        private static readonly SimpleCurve PointsCurve = new SimpleCurve
    {
        ...
        protected override bool TryExecuteWorker(IncidentParms parms)
        {
            ...
            Faction forcedFaction = Find.FactionManager.RandomEnemyFaction(allowNonHumanlike: false);
            int time = new IntRange(5000, 8500).RandomInRange;
            foreach (Pawn pawn in list)
            {
                Hediff hediff = pawn.health.AddHediff(VEE_DefOf.Traitor);
                HediffComp_Traitor comp = hediff.TryGetComp<HediffComp_Traitor>();
                if (comp != null) { 
                    comp.forcedFaction = forcedFaction;
                    comp.isGroup = true;
                    comp.ticksToDisappear = time;
                }
            }
