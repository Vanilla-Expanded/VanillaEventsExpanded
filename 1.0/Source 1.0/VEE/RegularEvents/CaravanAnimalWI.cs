    ...
    class CaravanAnimalWI : IncidentWorker
    {
        ...
        private List<PawnKindDef> PickPawnKindDef(Map map)
        {
            ...
            for (int i = 0; i < allPawnKindDefs.Count; i++)
            {
                ...
                if (pawnK.race.race.Animal && pawnK.race.race.packAnimal)
                {
                    float outdoorTemp = map.mapTemperature.OutdoorTemp;
                    if (outdoorTemp > pawnK.race.GetStatValueAbstract(StatDefOf.ComfyTemperatureMin, null) && outdoorTemp < pawnK.race.GetStatValueAbstract(StatDefOf.ComfyTemperatureMax, null))
                    {
