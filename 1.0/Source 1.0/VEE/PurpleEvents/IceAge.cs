    ...
    public class IceAge : GameCondition
    {
        ...
        public static void ChangeBiomes()
        {
            ...
            foreach (Tile tile in Find.World.grid.tiles)
            {
                ...
                if (tile.biome == VEE_DefOf.TemperateSwamp || tile.biome == VEE_DefOf.TropicalSwamp)
                {
                    tile.biome = VEE_DefOf.ColdBog;
                }
                if (tile.biome == BiomeDefOf.Desert || tile.biome == VEE_DefOf.AridShrubland || tile.biome == VEE_DefOf.ExtremeDesert)
                {
                    tile.biome = BiomeDefOf.Tundra;
                }
                if (tile.biome == BiomeDefOf.BorealForest || tile.biome == BiomeDefOf.Tundra || tile.biome == VEE_DefOf.ColdBog
